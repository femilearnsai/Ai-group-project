...
export const MessageBubble = ({ role, content, timestamp, onRegenerate, isLastAssistant, messageIndex, sessionId, isGreeting, onEdit, versions, currentVersionIndex, onVersionChange, sources, usedRetrieval }) => {
  ...
  return (
    <div className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 px-1 sm:px-0`}>
      <div className={`max-w-[95%] xs:max-w-[92%] sm:max-w-[88%] md:max-w-[85%] lg:max-w-[80%] px-2.5 sm:px-3 md:px-4 lg:px-5 py-2.5 sm:py-3 md:py-4 rounded-xl sm:rounded-2xl text-[13px] sm:text-sm leading-relaxed shadow-sm border ${
        isUser 
        ...
        <div className="flex items-center justify-between gap-1.5 sm:gap-2 mb-1.5 sm:mb-2 opacity-60">
            ...
            {!isUser && (
              <div className="flex items-center gap-0.5 sm:gap-1">
                <button
                  ...
                  disabled={isPlaying}
                  className={`p-0.5 sm:p-1 rounded transition-all ${
                    ttsError 
                ...
                >
                  {ttsError ? <AlertCircle size={10} className="sm:w-3 sm:h-3" /> : <Play size={10} className="sm:w-3 sm:h-3" />}
                </button>
                
                {isPlaying && (
                  <button
                    onClick={handlePauseVoice}
                    className="p-0.5 sm:p-1 rounded transition-all hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer"
                    title="Pause"
                  >
                ...
                {(isPlaying || isPaused) && (
                  <button
                    ...
                    className="p-0.5 sm:p-1 rounded transition-all hover:bg-rose-50 dark:hover:bg-rose-900/20 text-slate-600 dark:text-slate-300 hover:text-rose-500 dark:hover:text-rose-400 cursor-pointer"
                    title="Stop"
                  >
          ...
          {timestamp && (
            <span className="text-[7px] sm:text-[8px] font-bold tracking-wide">
